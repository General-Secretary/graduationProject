...
//         const url = `https://estethmarat-estethmarat794-2085-estethmarats-projects.vercel.app/api/v1/support-incubator/${id}`;
...
//                     <Link href={`/charaties/${post?.organizationId?._id}`} className='xs:w-2/3 my-5 w-[90%] xs:px-4 flex justify-center bg-[#00F5601F] bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-5 rounded-full items-center '>
...
//                     </Link>
//                     <div onClick={() => { setOpenFinanceOfferModal(true); }} className='w-full  xs:p-1 my-5 xs:w-1/3 px-4'>
...
//                 </div>
//                 <div className='flex flex-wrap justify-center xs:px-9 text-center items-center '>
//                     <Link href={`/incubators/${post?.organizationId?._id}`} className='xs:w-2/3 my-5 w-[90%] xs:px-4 flex justify-center bg-[#00F5601F] bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-5 rounded-full items-center '>
//                         <div className='w-1/6  xs:p-1'>
//                             <img src={post?.organizationId?.image.secure_url} className='w-full p-1 rounded-lg' alt="" />
//                         </div>
//                         <div className='w-5/12'><p className='font-semibold'>{post?.organizationId?.name}</p></div>
...
import { Button, Popover } from "flowbite-react";
import { Spinner } from "flowbite-react";
export default function CharityPostCard({ post, role, token  }) {
    const [selectedImage, setSelectedImage] = useState(null);
        const [offerContent, setOfferContent] = useState("");
    const [modalLoading, setModalLoading] = useState(false);
    const [openFinanceOfferModal, setOpenFinanceOfferModal] = useState(false);
    ...
    return (
        <div>
            ...
            <div className=' bg-gray-200  bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-5 w-[98%] xs:w-[97%]  mx-auto rounded-[50px]'>
                ...
                <div className='flex flex-wrap justify-center xs:px-9 text-center items-center '>
                    < Link href={`/charaties/${post?.organizationId?._id||post?.organizationId}`}  className='xs:w-2/3 my-5 w-[90%] xs:px-4 flex justify-center bg-[#00F5601F] bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-5 rounded-full items-center '>
                        <div className='w-1/6  xs:p-1'>
                            <img src={post?.organizationId?.image?.secure_url} className='w-full p-1 rounded-lg' alt="" />
                        </div>
                        <div className='w-5/12'><p className='font-semibold'>   {post?.organizationId?.name}   </p></div>
