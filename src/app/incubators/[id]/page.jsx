...
export default function page({ params }) {
    ...
    //                 fetch(`https://estethmarat-estethmarat794-gmailcom-estethmarats-projects.vercel.app/api/v1/supportOrganizations/${decodedId}`),
    ...
    const fetchUserDataAndCompany = async () => {
        try {
            // ✅ أولًا: تحميل بيانات الـ Incubator (متاحة للجميع)
            const [resIncubator, resIncubatorPosts] = await Promise.all([
                fetch(`https://estethmarat-estethmarat794-gmailcom-estethmarats-projects.vercel.app/api/v1/supportOrganizations/${decodedId}`),
                fetch(`https://estethmarat-estethmarat794-2085-estethmarats-projects.vercel.app/api/v1/posts/?organizationId=${decodedId}`),
            ]);
            ...
            // ✅ ثانيًا: تحميل بيانات المستخدم لو فيه توكن
            ...
            <div className="overflow-auto">
                <div className={` ${vazir.className}`}>
                            ...
                            <div className="  w-1/4 ms-auto">
                                <button disabled={!role}
                                    ...
                                    onClick={() => {
                                        sendRequest({
                                            profileId: decodedId,
                                            profileType: "SupportOrganization"
                                        })
                                    }}
                    ...
                    <div style={{ direction: "rtl" }} className="container p-5 mx-auto my-32 ">
                        ...
                        <SupportPostsForProfile role={role} token={token} posts={posts.posts} id={decodedId} />
                    </div>
                </div>
            </div>
        </div>
